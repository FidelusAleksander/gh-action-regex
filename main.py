...
class Action:
    ...
    def from_environ(cls):
        return cls(regex_pattern=os.environ["INPUT_REGEX_PATTERN"], text=os.environ["INPUT_TEXT"],
                   regex_match_type=os.environ["INPUT_REGEX_MATCH_TYPE"])

    def _validate_input(self):
        if self.REGEX_MATCH_TYPE not in self.SUPPORTED_REGEX_MATCH_TYPES:
            print(f"Regex match type {self.REGEX_MATCH_TYPE} has to be one of {self.SUPPORTED_REGEX_MATCH_TYPES}")
            sys.exit(1)
    ...
    def run(self):
        result = getattr(re, self.REGEX_MATCH_TYPE)(pattern=self.REGEX_PATTERN, string=self.TEXT)
